 ...
 * collapsing logo strip) that was sized for a five-letter name: "Supacharge"
 * at that size is ~370 px wide and was cut to "Supac". The SDK owns that
 * box, so the fix lives here: after mount the wordmark measures the nearest
 * ancestor that clips horizontally and, only when it does not fit, scales
 * itself down with a CSS transform (origin left) to the room it has. Same
 * font, same weight, same classes — nothing changes where nothing clips
 * (the landing header, lms_sdk's footer).
 */
