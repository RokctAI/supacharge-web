...
// already renders these same base_sdk / lms_sdk components - so the two
...
//     webfont in the root layout, and lms_sdk's landing sets its own faces
//     through `--sc-font-*` in lms-theme.css, so Tailwind's default stacks
//     are what `font-sans`/`font-mono` should resolve to here.
//   * `primary` reads `hsl(var(--primary))` rather than rokctai_frontend's
//     hardcoded wealth-green; the variable is Supacharge's orange in
//     app/globals.css. The `wealth-green` scale and the `marquee` keyframes
//     are rokctai-only (zero uses in this tree), so they are not carried.
