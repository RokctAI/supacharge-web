...
export function LmsPricing({
  ...
  // The same test as showsPricing() in ./lms-pricing.tsx, the pure rule
  // `meta.renders` asks there so the host drops the section and its nav
  // stop together; repeated inline so this client half imports nothing
  // from its entry (the compiler narrows `config` on the test, too).
