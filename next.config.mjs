...
const nextConfig = {
  ...
  env: {
    ...
    //
    // supacharge-web serves exactly ONE tenant. It knows its backend from
    // ROKCT_BASE_URL / NEXT_PUBLIC_ROKCT_BASE_URL, so there is nothing for a
    // database to tell it and nothing about one user that another deployment
    // would need to look up. "single-tenant" selects
    ...
    // returns null, so with no POSTGRES_URL set every login here failed as
